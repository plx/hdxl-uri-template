...
| [`ARCH-03`](Evidence/2026-07-27-arch-03-expansion-error-call-graph.md) | [#37 — Remove unreachable expansion error types and simplify throwing boundaries](https://github.com/plx/hdxl-uri-template/issues/37) | P2 task |
...
Epic [#7](https://github.com/plx/hdxl-uri-template/issues/7) also remains
blocked by `QA-03` and `DOC-01`. The three `HARD-*` children complete its
production fixes, but #7 stays open until `QA-03` makes the temporary
sanitizer, fuzz, concurrency, and adversarial-performance probes permanent and
`DOC-01` publishes the user-facing delimiter-injection and
unsafe-resulting-scheme guidance required by the epic's exit criteria.
