...
@Suite(.tags(.variableExpansion,.uriVariableListValue))
struct URIVariableListValueExpansionTests {

  @Test
  private func `count examples from RFC (direct, no parsing)`() throws {
  ...
  @Test
  private func `count examples from RFC (parsed)`() throws {
    let templates = [
      "{count}",
      "{count*}",
      "{/count}",
